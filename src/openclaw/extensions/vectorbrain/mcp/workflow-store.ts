...
export async function createWorkflow(goal: string, plan?: string): Promise<Workflow> {
  await initStore();
  const wf: Workflow = {
    workflow_id: id('wf'),
    goal,
...
export async function getWorkflow(wfId: string): Promise<Workflow | null> {
  return readJson<Workflow>(path.join(WORKFLOWS_DIR, `${wfId}.json`));
}

export async function updateWorkflow(wfId: string, updates: Partial<Workflow>): Promise<Workflow | null> {
...
export async function listWorkflows(status?: WorkflowStatus): Promise<Workflow[]> {
  const files = await listFiles(WORKFLOWS_DIR, '.json');
  const wfs = await Promise.all(files.map(f => readJson<Workflow>(f)));
  if (status) return wfs.filter((w): w is Workflow => !!w && w.status === status);
  return wfs.filter((w): w is Workflow => w !== null).sort((a, b) =>
...
export async function getTask(taskId: string): Promise<Task | null> {
  return readJson<Task>(path.join(TASKS_DIR, `${taskId}.json`));
}
  ...
  if (updates.status === 'done' || updates.status === 'passed') {
    ...
    if (wf) {
      const doneCount = await Promise.all(
        wf.tasks.map(tid => getTask(tid))
      ).then(tasks => tasks.filter(t => t && (t.status === 'done' || t.status === 'passed')).length);
...
export async function getRunnableTasks(wfId: string): Promise<Task[]> {
  const wf = await getWorkflow(wfId);
  if (!wf) return [];
...
export async function cancelWorkflow(wfId: string, reason = '用户取消'): Promise<Workflow | null> {
  const wf = await getWorkflow(wfId);
  if (!wf) return null;
...
export async function pauseWorkflow(wfId: string): Promise<Workflow | null> {
  const wf = await getWorkflow(wfId);
...
export async function resumeWorkflow(wfId: string): Promise<Workflow | null> {
  const wf = await getWorkflow(wfId);
...
export async function cancelTask(taskId: string, reason = '用户取消'): Promise<Task | null> {
...
export async function accumulateCost(wfId: string, costUsd: number): Promise<number> {
  const wf = await getWorkflow(wfId);
  if (!wf) return 0;
...
export async function getAccumulatedCost(wfId: string): Promise<number> {
  const wf = await getWorkflow(wfId);
...
export async function appendLog(entry: Omit<LogEntry, 'log_id' | 'timestamp'>): Promise<LogEntry> {
  await initStore();
...
export async function getLogs(wfId: string, taskId?: string): Promise<LogEntry[]> {
  ...
  for (const file of files) {
    ...
    for (const line of lines) {
      try {
        ...
        if (e.workflow_id !== wfId) continue;
        if (taskId && e.task_id !== taskId) continue;
        entries.push(e);
      } catch {
...
export async function getWorkflowSummary(wfId: string): Promise<{
  ...
  if (!workflow) return { workflow: null, tasks: [], logs: [] };
  const tasks = await Promise.all(workflow.tasks.map(tid => getTask(tid)));
  return { workflow, tasks: tasks.filter(t => t !== null) as Task[], logs };
}
