...
import { Task, LogEntry, appendLog } from './workflow-store.js';
 ...
 */
export function detectRelevantSkills(taskTitle: string, taskDescription?: string): string[] {
...
function runClaudeCLI(
  ...
  return new Promise((resolve) => {
    ...
    const stdinOk = proc.stdin?.write(prompt, (err: Error | null | undefined) => {
      if (err && !finished) {
        finished = true;
        cleanup();
...
export function getSkillRegistry(): string {
  return `【可用技能注册表】
...
export function buildSkillContent(skillNames: string[]): string {
  if (!skillNames.length) return '';
  const sections: string[] = ['【技能文档】\n'];
...
export function getSpecReviewerPrompt(): string {
  return `${getSkillRegistry()}
...
export function getCodeQualityReviewerPrompt(): string {
  return `${getSkillRegistry()}
...
function parseCost(output: unknown): DispatchResult['cost'] {
  ...
  if ('result' in obj && typeof obj.result === 'string') {
    try {
      ...
      if (typeof inner === 'object' && inner !== null) {
        return extractFromObj(inner as Record<string, unknown>);
      }
    } catch { /* fall through */ }
  }
...
function extractFromObj(obj: Record<string, unknown>): DispatchResult['cost'] {
