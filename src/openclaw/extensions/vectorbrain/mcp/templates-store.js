...
function id(prefix = 'tmpl') {
    return `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
}
export async function listTemplates() {
    await ensureDir(TEMPLATES_DIR);
    const files = await listFiles(TEMPLATES_DIR, '.json');
    const templates = await Promise.all(files.map(f => readJson(f)));
    return templates.filter(t => t !== null).sort((a, b) => b.last_used_at?.localeCompare(a.last_used_at ?? '') ?? b.created_at.localeCompare(a.created_at));
}
export async function getTemplate(templateId) {
    return readJson(path.join(TEMPLATES_DIR, `${templateId}.json`));
}
