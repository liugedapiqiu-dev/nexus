 ...
 * 3. 全链路日志，输出汇总报告
 */
import { createWorkflow, getWorkflow, updateWorkflow, createTask, getTask, updateTask, getRunnableTasks, listWorkflows, appendLog, getLogs, accumulateCost, getAccumulatedCost, } from './workflow-store.js';
import { dispatchTask, getImplementerPrompt, getSpecReviewerPrompt, getCodeQualityReviewerPrompt, detectRelevantSkills, } from './dispatcher.js';
import { sleep, getVbDir } from './utils.js';
export async function orchestrate(opts) {
    const startTime = Date.now();
    const { goal, tasks: taskDefs, workflowId: existingWfId, model, maxAttempts = 3, maxCostUsd, costAlertThreshold, onCostAlert, onProgress, autoRemember = true, deduplicate = true, } = opts;
    ...
    if (existingWfId) {
        ...
        if (!existing)
            throw new Error(`Workflow not found: ${existingWfId}`);
        wf = existing;
    }
    else {
    ...
    while (true) {
        ...
        if (runnable.length === 0 && completedCount < taskDefs.length) {
            ...
            const blocked = pending.filter(t => t.depends_on.some(depId => allTasks.some(x => x?.task_id === depId && (x?.status === 'failed' || x?.status === 'cancelled'))));
            if (blocked.length > 0) {
        ...
        for (const group of byDepth) {
            ...
            const results = await Promise.all(group.map(task => runTask(wf.workflow_id, task, { model, maxAttempts, signal: workflowController.signal })));
            for (const [task, result] of zip(group, results)) {
                ...
                if (result.status === 'done' || result.status === 'passed') {
                    completedCount++;
                    if (result.output)
                        keyOutputs.push(extractKeyOutput(result.output));
                    onProgress?.({
                        workflow_id: wf.workflow_id,
                        task_id: task.task_id,
...
async function runTask(workflowId, task, opts) {
...
// ─── 辅助函数 ────────────────────────────────────────────────────────────────
function groupByDepth(tasks) {
    const depthMap = new Map();
    const result = [];
    const taskMap = new Map(tasks.map(t => [t.task_id, t]));
    const visiting = new Set();
    function depth(t) {
        if (depthMap.has(t.task_id))
            return depthMap.get(t.task_id);
...
function extractPass(output) {
    ...
    if (typeof output === 'object') {
        ...
        if ('result' in obj && typeof obj.result === 'string') {
            try {
                ...
                if (typeof inner === 'object' && inner !== null) {
                    if ('passed' in inner)
                        return Boolean(inner.passed);
                    if ('pass' in inner)
                        return Boolean(inner.pass);
                    if ('success' in inner)
                        return Boolean(inner.success);
                }
        ...
        if ('pass' in obj)
            return Boolean(obj.pass);
        if ('success' in obj)
            return Boolean(obj.success);
    }
    return false;
}
function extractKeyOutput(output) {
    if (!output)
        return '';
    ...
    if (typeof obj === 'object' && obj !== null) {
        if ('result' in obj) {
            ...
            if (typeof r === 'object' && r !== null) {
                const ro = r;
                if ('summary' in ro)
                    return String(ro.summary).slice(0, 100);
                if ('text' in ro)
                    return String(ro.text).slice(0, 100);
            }
        }
        if ('summary' in obj)
            return String(obj.summary).slice(0, 100);
        if ('text' in obj)
            return String(obj.text).slice(0, 100);
    }
...
async function findSimilarWorkflow(goal) {
...
async function autoWriteMemory(workflowId, goal, status, completed, total, duration_ms, keyOutputs) {
    try {
