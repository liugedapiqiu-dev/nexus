 ...
 */
import { listTemplates, saveTemplate, deleteTemplate, getTemplate, BUILTIN_TEMPLATES, } from '../mcp/templates-store.js';
export default {
    ...
    async execute(_toolCallId, opts, _signal) {
        switch (opts.action) {
            ...
            case 'get': {
                if (!opts.template_id)
                    return { success: false, message: 'get 需要 template_id 参数' };
            ...
            case 'delete': {
                if (!opts.template_id)
                    return { success: false, message: 'delete 需要 template_id 参数' };
