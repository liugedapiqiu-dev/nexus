...
async function main() {
  try {
    ...
    try { if (sessionFile) fs.unlinkSync(sessionFile); } catch {}
    process.exit(0);
  }
}

async function generateReport(result: any, reportDir: string): Promise<string> {
  const lines: string[] = [
    `# 工作流执行报告`,
    ``,
...
async function sendFeishuNotification(result: any, goal: string): Promise<void> {
  try {
