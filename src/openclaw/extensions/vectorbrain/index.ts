...
function postJson(path: string, payload: AnyRecord, timeoutMs = 1200): Promise<{ statusCode: number; body: string }> {
...
function normalizeMessage(msgCtx: any) {
...
async function sendUnifiedPreprocess(message: ReturnType<typeof normalizeMessage>) {
...
export default function register(api: any) {
  ...
  api.on('message_received', async (msgCtx: any) => {
    if (!toolsReady) return; // wait for tools to register
