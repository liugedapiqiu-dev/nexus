 ...
 * 完成后自动发送飞书通知 + 生成报告文件。
 */
export function runInBackground(opts: BackgroundRunOptions): { workflow_id: string; pid: number } {
  const {
    goal, tasks, model, maxAttempts = 3,
    notify = 'feishu',
    reportPath,
  } = opts;
 ...
 */
export async function cleanupOldSessions(maxAgeMs = 24 * 60 * 60 * 1000): Promise<void> {
  try {
